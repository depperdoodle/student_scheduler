...
def _day_steps(conn, week_start, day, day_blocks, students, scheduled_ids, scan_increment):
    ...
    for block in day_blocks:
        ...
        events = conn.execute(
            """SELECT start_time, end_time FROM event_blocks
               WHERE week_start=? AND day_of_week=?""",
            (week_start, day),
        ).fetchall()
        occupied = sorted(
            [(time_to_min(r["start_time"]), time_to_min(r["end_time"])) for r in remaining]
            + [(time_to_min(r["start_time"]), time_to_min(r["end_time"])) for r in events]
        )
...
def generate_schedule_for_week(week_start, scan_increment=5):
    ...
    Labeled event blocks (meetings, lunch, travel, etc., added directly on
    the Schedule page) reserve their time the same way, across every
    school that day, so no student session gets auto-generated on top of
    one.
    """
    conn = get_db()
